...
// columnas, enlace de WhatsApp, email, duplicados, campo trampa, validaciones y fórmulas.
const fs = require('fs');
const vm = require('vm');
...
const base = { nombre: 'Laura Gómez', telefono: '+34 612 345 678', perfil: 'Fisioterapeuta', equipo: 'Ecógrafo', modelo: 'Acclarix AX8 (EDAN)', consentimiento: 'Sí · 2026-09-24T10:00:00Z', utm_source: 'facebook', utm_medium: 'paid', utm_campaign: 'test', utm_content: '', utm_term: '', fbclid: 'abc', fbc: 'fb.1.1.abc', fbp: 'fb.1.2.3', referrer: '', landing_url: 'https://x/?utm_source=facebook', dispositivo: 'Móvil · iOS · Instagram', idioma: 'es-ES', event_id: '11111111-1111-4111-8111-111111111111', website: '' };
const res = [];
const ok = (c, n, x = '') => res.push(`${c ? 'PASS' : 'FAIL'}  ${n}${x ? '  · ' + x : ''}`);

{ const { ctx, sheets } = makeEnv(); ctx.setup(); const sh = sheets.Leads; ok(sh && sh.rows[0].join('|') === 'Fecha|Nombre|Teléfono|WhatsApp|Perfil|Equipo|Modelo|Consentimiento|utm_source|utm_medium|utm_campaign|utm_content|utm_term|fbclid|fbc|fbp|Referrer|URL de entrada|Dispositivo|Idioma|event_id|Estado' && sh.frozen === 1, 'setup(): crea "Leads" con las cabeceras y la primera fila fija', sh && sh.rows[0].join(', ')); }
